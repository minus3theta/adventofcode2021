...
fn solve(input: &str) -> anyhow::Result<i32> {
    let mut scanners = parse(input)?;
    let first = scanners.remove(0);
    let mut all_beacon = first.into_iter().collect::<BTreeSet<_>>();
    let mut all_offset = vec![Coord(0, 0, 0)];
    while !scanners.is_empty() {
        let (found, not_found): (Vec<_>, Vec<_>) = scanners.into_iter().partition_map(|scanner| {
            let (count, shifted, offset) = ALL_BASIS
                .iter()
                .map(|basis| {
                ...
                .unwrap();
            if count >= 12 {
                Either::Left((shifted, offset))
            } else {
                Either::Right(scanner)
            }
        });
        all_offset.extend(found.iter().map(|&(_, offset)| offset));
        all_beacon.extend(found.into_iter().flat_map(|(sc, _)| sc.into_iter()));
        scanners = not_found;
    }
    Ok(all_offset
        .iter()
...
fn count_match(fixed: &BTreeSet<Coord>, target: &[Coord]) -> (usize, Scanner, Coord) {
    fixed
        .iter()
        ...
        .map(|(&c, &t)| {
            ...
            let overlap = shifted.iter().filter(|&t| fixed.contains(t)).count();
            (overlap, shifted, diff)
        })
        .max()
        .unwrap()
}
...
fn distance(lhs: Coord, rhs: Coord) -> i32 {
    let Coord(x, y, z) = lhs - rhs;
    x.abs() + y.abs() + z.abs()
}
...
-652,-548,-490
30,-46,-14";
        assert_eq!(solve(input).unwrap(), 3621);
    }
}
