...
fn solve(target: Area) -> i64 {
    let ((_, x1), (y0, _)) = target;
    (0..=x1)
        .flat_map(|vx| (0..=y0.abs()).map(move |vy| top(target, vx, vy)))
        .flatten()
        .max()
        .unwrap()
}

fn top(target: Area, mut vx: i64, mut vy: i64) -> Option<i64> {
    let mut x = 0;
    let mut y = 0;
    let mut max_y = 0;
