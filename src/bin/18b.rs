...
fn solve(input: &str) -> anyhow::Result<i64> {
    fn go(mut x: Number, y: Number) -> i64 {
        x += y;
        x.reduce();
        x.magnitude()
    }
    ...
    let mut ans = 0;
    for (x, y) in nums.iter().tuple_combinations() {
        ans = ans.max(go(x.clone(), y.clone()));
        ans = ans.max(go(y.clone(), x.clone()));
    }
...
[[[[5,2],5],[8,[3,7]]],[[5,[7,5]],[4,4]]]";
        assert_eq!(solve(input).unwrap(), 3993);
    }
}
